...
import { CartCategory, Product } from "@/lib/types";
      ...
      <div className="bg-white rounded-[32px] w-full max-w-sm p-6 shadow-2xl overflow-hidden relative">
        ...
        <div className="h-40 relative flex items-center justify-center">
          <AnimatePresence mode="wait">
            {currentDiff && phase === "analyzing" && (
              ...
              >
                <div className="relative">
                  <img src={currentDiff.oldItem.image} alt={currentDiff.oldItem.name} className="w-20 h-20 rounded-2xl object-cover shadow-md" />
                  <motion.div
                ...
                {currentDiff.changed ? (
                  <div className="flex items-center space-x-4">
                    <div className="flex flex-col items-center opacity-50 grayscale">
                      <img src={currentDiff.oldItem.image} alt={currentDiff.oldItem.name} className="w-14 h-14 rounded-xl object-cover" />
                    </div>
                    <ArrowRight className="text-red-500" size={24} />
                    <div className="flex flex-col items-center">
                      <img
                        src={currentDiff.newItem.image}
                        alt={currentDiff.newItem.name}
                        className="w-20 h-20 rounded-2xl object-cover shadow-lg border-2 border-red-500"
                      />
            ...
            {phase === "done" && (
              <motion.div key="done" initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center">
